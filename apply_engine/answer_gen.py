...
Status per answer:
  ...
  drafted   : essay draft in Sam's voice, passed the gate — FILL but flag for review
  declined  : the drafter returned DECLINE (fact not supported) — left for Sam
  blocked   : the gate caught a forbidden/overstated phrase — left for Sam
"""
import json
import re
...
# ground Sam's real constraints (where he lives, what he'll commit to, his start date,
# pay expectations), and a wrong commitment is worse than a blank. These are always left
# for Sam. Matched case-insensitively against the question text. Kept tight so genuine
...
def is_personal_commitment(question: str) -> bool:
    """True for logistics/commitment/availability/pay questions only Sam can answer.
...
def is_office_auto_yes(question: str) -> bool:
    """True for in-office / RTO / relocation questions the policy answers YES (not Sam's to
...
def build_prompt(question, kind: str, facts: str, used_examples: str = "") -> str:
    ...
    The FACTS blob carries a VOICE & CRAFT section (who Sam is + how he writes)
    ...
    """
    base = (
        "You are Sam Rivera, writing your own answer to a job-application question. "
...
def build_refine_prompt(question, draft: str, facts: str, used_examples: str = "") -> str:
    ...
    """
    return (
        "You are Sam Rivera's writing editor. Below is a DRAFT answer to a job-application "
        ...
        "- Keep first person and Sam's warm, confident, plain voice. Active voice throughout.\n"
...
def build_critique_prompt(question, draft: str, facts: str, used_examples: str = "") -> str:
    ...
    A strict reviewer reads the draft the way a sharp hiring manager (and Sam) would and
    ...
    return (
        "You are a demanding reviewer of Sam Rivera's job-application answer — both a sharp "
        "hiring manager and Sam himself, who has zero tolerance for generic filler. Read the "
        ...
        "- It is flat, padded, cliché, or makes no real point about how Sam thinks or works.\n"
...
def build_revise_with_critique_prompt(question, draft: str, critique: str, facts: str,
    ...
    return (
        "You are Sam Rivera, revising your own job-application answer after a sharp reviewer "
        ...
        "- Keep first person and Sam's warm, confident, plain voice; active voice; at most one "
        "em-dash.\n"
...
PACKAGE_CRITIC_LENSES = [
    ...
    ("voice/authenticity",
     "Flag any answer that reads as robotic, cliché, padded, or off Sam's plain, warm, confident "
     "voice — anything that sounds AI-generated rather than like Sam actually wrote it."),
]
...
def build_package_critique_prompt(lens_name: str, lens_instruction: str, items, facts: str) -> str:
    ...
    return (
        f"You are reviewing Sam Rivera's drafted job-application answers through ONE lens: "
        f"{lens_name}.\n{lens_instruction}\n\n"
...
def generate(questions, facts: str, llm_fn, audit_fn, *,
    ...
    for q in questions:
        # Logistics / personal-commitment / pay questions are Sam's to answer — the
        ...
        # answers don't drive the repetition Sam flagged).
        if q.kind == "essay":
...
def draft_single_call(questions, facts: str, agent_fn, audit_fn, *, repair: bool = True) -> list:
    ...
    injectable for tests). Robustness — a single call is a single point of failure, so:
      ...
      * escalate the whole set to Sam (status='declined') if even the retry won't parse —
    ...
    if askable:
        ...
        for i, q in enumerate(askable, start=1):
            if not by_n:
                # Total parse failure even after repair → escalate the whole set to Sam.
                out_by_q[q.label] = Answer(q.label, q.selector, q.kind, status="declined",
                                           reason="single-call could not parse answers — needs Sam")
                continue
