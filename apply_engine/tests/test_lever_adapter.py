...
def test_lever_fill_readback_and_auth(fixture_server, answers, tmp_path):
    a = LeverAdapter()
    with launch_profile(headless=True, profile_dir=tmp_path / "p") as (ctx, page):
        page.goto(f"{fixture_server}/lever_form.html")
        ...
        assert observed["linkedin"] == "https://linkedin.com/in/jb"
